...
const Snake = () => {
  ...
  useEffect(() => {
    ...
    ctx.fillStyle = 'red';
    ctx.fillRect(tileSize * (appleX + 0.5) - 20,
      tileSize * (appleY + 0.5) - 20, spacedWidth, spacedWidth);
    // snake
    ...
    let [x, y] = snake[0];
    ctx.moveTo(tileSize * (x + 0.5 + 0.5 * dx) - (dx * spacing),
      tileSize * (y + 0.5 + 0.5 * dy) - (dy * spacing));
  ...
  const moveSnake = () => {
    ...
    if (x2 === appleX && y2 === appleY) {
      ...
      for (let i = 0; i < 20; i++) {
        for (let j = 0; j < 20; j++) {
          const found = nextSnake.find(([x, y]) => x === i && y === j);
          if (!found) {
            eligPoints.push([i, j]);
          }
        }
